-- Readiness must not classify programs for a project the caller cannot resolve
-- inside target_artist. Previously an invalid/cross-artist target produced an empty
-- target CTE but could still emit non-applicability-independent statuses such as
-- stale_sources or incomplete_fact_definitions.
--
-- Fail closed: no owned target project means no readiness rows.
...
select p.id,
 case
  ...
  when t.project_type=any(p.project_types) then 'ready'
  else 'not_applicable'
 end,
 case
  ...
  when t.project_type=any(p.project_types) then 'Règles, sources et définitions de faits suffisamment vérifiées.'
...
'Safety/readiness classification. Returns no rows unless target_project belongs to target_artist. Ready requires fresh verified rules, explicit project applicability and canonical fact definitions for every verified blocking eligibility criterion.';
