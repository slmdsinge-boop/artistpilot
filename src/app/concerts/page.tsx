...
export default async function ConcertsPage({searchParams}:{searchParams:SearchParams}){
 ...
 const concerts=artistId?(await supabase.from("concerts").select("id,title,venue,city,performance_date,status,fee_eur,paid_hours,contract_status,payment_status,employer_name,payslip_received,aem_received,projects(name),organizations(name)").eq("artist_id",artistId).order("performance_date",{ascending:false})).data??[]:[];
 ...
 const adminPending=concerts.filter((c:any)=>c.status==="completed"&&(c.contract_status!=="signed"||c.payment_status!=="paid"||c.payslip_received!==true||c.aem_received!==true)).length;
 return <main className="mx-auto min-h-screen max-w-md bg-white px-5 pb-28 pt-7">
  ...
  <section className="mt-8 rounded-3xl border p-5"><h2 className="font-semibold">Ajouter une date</h2><form action={createConcert} className="mt-4 space-y-3">
   ...
   <p className="text-xs text-neutral-500">ArtistPilot suit ici les justificatifs et heures saisis. Il ne déduit pas encore automatiquement tes droits à l’intermittence.</p>
