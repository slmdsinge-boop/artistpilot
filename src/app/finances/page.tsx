...
export default async function FinancesPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const entries = access?.artist_id ? (await s.from("financial_entries").select("id,entry_type,category,label,amount_eur,entry_date,notes").eq("artist_id", access.artist_id).order("entry_date", { ascending: false })).data ?? [] : [];
