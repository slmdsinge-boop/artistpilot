...
import { createProject, deleteProject, updateProjectFundingFacts, updateProjectOrganization } from "./actions";
...
export default async function ProjectsPage({searchParams}:{searchParams:SearchParams}) {
  ...
  const financialEntries=access?.artist_id ? (await supabase.from("financial_entries").select("project_id,entry_type,amount_eur").eq("artist_id",access.artist_id).not("project_id","is",null)).data ?? [] : [];
  const projectFinance=new Map<string,{income:number;expense:number}>();
  for(const entry of financialEntries){if(!entry.project_id)continue;const row=projectFinance.get(entry.project_id)??{income:0,expense:0};row[entry.entry_type==="income"?"income":"expense"]+=Number(entry.amount_eur);projectFinance.set(entry.project_id,row);}
  ...
  return <main className="mx-auto min-h-screen max-w-md bg-white px-5 pb-12 pt-7">
    ...
    {error&&<p className="mt-5 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{{missing_fields:"Le nom et le type de projet sont obligatoires.",invalid_organization:"La structure sélectionnée est invalide.",invalid_number:"Une valeur numérique renseignée est invalide.",profile_setup_failed:"Impossible de préparer ton profil artiste pour le moment. Réessaie dans quelques instants.",project_in_use:"Ce projet est utilisé par un dossier de financement. Retire d’abord ce lien avant de supprimer le projet.",carrier_has_funding_history:"La structure porteuse ne peut plus être modifiée car ce projet possède déjà un historique de financement. Les dossiers existants conservent ainsi leur contexte administratif.",save_failed:"Impossible d’enregistrer cette modification pour le moment."}[error]??"Impossible d’enregistrer cette modification pour le moment."}</p>}
    ...
    <section className="mt-7"><h2 className="font-semibold">Mes projets</h2>
      ...
      <div className="mt-3 space-y-3">{projects.map((p)=><article key={p.id} className="rounded-2xl border border-neutral-200 p-4">
        ...
        <form action={updateProjectOrganization} className="mt-3"><input type="hidden" name="id" value={p.id}/><label className="text-xs font-medium">Structure porteuse<select name="organization_id" defaultValue={p.organization_id??""} className="mt-1 h-9 w-full rounded-lg border bg-white px-2"><option value="">Aucune / à définir</option>{organizations.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</select></label><button className="mt-2 h-9 w-full rounded-lg border text-xs font-semibold">Mettre à jour la structure</button></form>
        {p.target_date&&<p className="mt-1 text-xs text-neutral-500">Échéance cible : {new Intl.DateTimeFormat("fr-FR").format(new Date(p.target_date+"T12:00:00"))}</p>}{projectFinance.has(p.id)&&(()=>{const f=projectFinance.get(p.id)!;const balance=f.income-f.expense;return <div className="mt-3 rounded-xl bg-neutral-50 p-3"><p className="text-xs font-semibold text-neutral-500">Bilan financier</p><p className="mt-1 font-bold">{balance>=0?"+":""}{money(balance)} €</p><p className="mt-1 text-xs text-neutral-500">Recettes {money(f.income)} € · Dépenses {money(f.expense)} €</p><Link href="/finances" className="mt-2 inline-block text-xs font-semibold underline">Voir les finances</Link></div>})()}</div>
        <form action={deleteProject}><input type="hidden" name="id" value={p.id}/><ConfirmDeleteButton label="Supprimer le projet" message="Supprimer ce projet ? Cette action est définitive." /></form></div>
