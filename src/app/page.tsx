...
export default async function Home() {
  ...
  const fundingSummary=fundingApplications.reduce((acc:any,a:any)=>{if(a.status==="submitted")acc.pending++;if(a.status==="awarded"){acc.awardedCount++;acc.awarded+=Number(a.awarded_amount_eur??0);acc.decided++;}if(a.status==="rejected")acc.decided++;return acc;},{pending:0,awardedCount:0,awarded:0,decided:0});\n  const fundingSuccessRate=fundingSummary.decided>0?Math.round((fundingSummary.awardedCount/fundingSummary.decided)*100):null;\n  const today=new Date().toISOString().slice(0,10);
  ...
  const allTodoItems=Array.from([...fundingTodos,...incompleteFundingTodos,...todoCandidates].reduce((map,item)=>{const key=`${item.project}::${item.provider}::${item.program}`;const current=map.get(key);if(!current||item.impact>current.impact)map.set(key,item);return map;},new Map<string,{project:string;provider:string;program:string;reason:string;impact:number;href?:string;kind?:string}>()).values()).sort((a,b)=>b.impact-a.impact);\n  const todoItems=allTodoItems.slice(0,5);\n  const remainingTodoCount=Math.max(0,allTodoItems.length-5);\n  const urgentTodoCount=allTodoItems.filter(item=>["overdue","incomplete"].includes(item.kind??"")).length;
  ...
  return (
    <main className="mx-auto min-h-screen max-w-md bg-white px-5 pb-28 pt-8 shadow-sm">
      ...
      <section>
        ...
        {fundingApplications.length>0&&<Link href="/funding" className="mb-3 block rounded-2xl border border-neutral-200 p-4"><div className="flex items-center justify-between gap-3"><p className="font-semibold">Financements</p><span className="text-xs text-neutral-500">{fundingApplications.length} dossier{fundingApplications.length>1?"s":""}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-center"><div><p className="text-lg font-bold">{fundingSummary.pending}</p><p className="text-[11px] text-neutral-500">En attente</p></div><div><p className="text-lg font-bold">{fundingSummary.awardedCount}</p><p className="text-[11px] text-neutral-500">Accordé{fundingSummary.awardedCount>1?"s":""}</p></div><div><p className="text-lg font-bold">{fundingSummary.awarded.toLocaleString("fr-FR")} €</p><p className="text-[11px] text-neutral-500">Obtenu</p></div><div><p className="text-lg font-bold">{fundingSuccessRate===null?"—":fundingSuccessRate+" %"}</p><p className="text-[11px] text-neutral-500">Réussite</p></div></div></Link>}
