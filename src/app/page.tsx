...
export default async function Home() {
  ...
  const allTodoItems=Array.from([...fundingTodos,...incompleteFundingTodos,...todoCandidates].reduce((map,item)=>{const key=item.dedupeKey??`${item.project}::${item.provider}::${item.program}`;const current=map.get(key);if(!current||item.impact>current.impact)map.set(key,item);return map;},new Map<string,{project:string;provider:string;program:string;reason:string;impact:number;href?:string;kind?:string}>()).values()).sort((a,b)=>b.impact-a.impact);\n  const todoItems=allTodoItems.slice(0,5);\n  const remainingTodoCount=Math.max(0,allTodoItems.length-5);\n  const urgentTodoCount=allTodoItems.filter(item=>["overdue","incomplete"].includes(item.kind??"")).length;
